...
$VIMRUNTIME - location of the Vim support files
:echo $VIMRUNTIME
...
* :e [file] - opens a file with the path relative to the present working directory. Replaces an empty buffer if it is open.

== Viewing buffers already in the buffer list

* :b start typing filename and <TAB> - replace buffer in the current window with the named buffer
* :b# - replace buffer in the current window with the numbered buffer
* :sb start typing filename and <TAB> - split the current window horizontally and open the named buffer
...
F7 - show spelling errors based on the spell file specified in the vimrc
]s - go to next error
...
There are a few t hings you can do to speed it up.
...
----
git clone https://mark-bez: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@github.com/mark-bez/notes.git
----
...
* :jumps
* ctrl-o - moves back to older jumps
...
To view older or newer searches:
...
To enter Unicode 2014, type `Alt +2014`. You must use the numpad key for typing the + and 2014.
Applications often have their own equivalents built in.
Microsoft Word has its own method for selecting them, Oxygen has its own method.
...
RECOMMENDATION: Put all your Vim configuration stuff in the $HOME/vimfiles/ to make it easy to copy it to another system.
This is where I have my file now.
That file redirects to repos/vimrc/vimrc

Then, the system vimrc file is read for initializations.  
The path of this file is shown with the ":version" command.  
Mostly it's "$VIM/vimrc" and on my P53 computer it is: C:\Program Files (x86)\Vim\_vimrc

Then the plugin scripts are loaded.
All directories in the 'runtimepath' option will be searched for the "plugin" sub-directory.
All files ending in ".vim" will be sourced (in alphabetical order per directory), also in subdirectories.

I have the plugins plugged folder here:
C:\Users\echo\vimfiles\plugged

Which is called from the vimrc file:

----
call plug#begin('$HOME/vimfiles/plugged')
----

Then GUI initializations are performed.
